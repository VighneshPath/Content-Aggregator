parts_sites = {
    ...
    "+":[("www.amazon.in","https://www.amazon.in/s?k=item goes here"),("www.mdcomputers.in","https://mdcomputers.in/index.php?category_id=0&search=item goes here&submit_search=&route=product%2Fsearch"),("www.primeabgb.com","https://www.primeabgb.com/?post_type=product&taxonomy=product_cat&s=item goes here")]
}
...
def mdcomp(soup, part_name, site):
    ...
    for item in results:
        try:
            title = item.find("div", {"class":"right-block right-b"}).h4.a.get_text().strip()
            price = item.find("span",{"class":"price-new"}).get_text().strip()
            link = item.find("div", {"class":"right-block right-b"}).h4.a['href'].strip()
            img_link = item.find("div",{"class":"product-image-container"}).a.img["data-src"]
            flag = 0
...
def amazon(soup, part_name, site):
    ...
    for item in results:
        try:
            ...
            img_link = item.find("div",{"class":"a-section aok-relative s-image-fixed-height"}).img["src"]
            flag = 0
            for word in part_name.split(" "):
                if(word not in title.lower().split()):
                    flag = 1
...
def vedcomp(soup, part_name, site):
    ...
    for item in results:
        try:
            ...
            try:
                img_link = item.find("img",{"class":"lazy first-image"})["data-src"]
            except:
                img_link = item.find("img",{"class":"lazy first-image"})["data"]
            flag = 0
            for word in part_name.split(" "):
                if(word not in title.lower().split()):
                    flag = 1
...
def primeabgb(soup, part_name, site):
    ...
    for item in results:
        try:
            title = item.find("div",{"class":"product-innfo"}).h3.a.get_text().strip()
            try:
                try:
                ...
                continue
            link = item.find("div",{"class":"product-innfo"}).h3.a['href'].strip()
            img_link = item.find("img",{"class":"attachment-post-thumbnail wp-post-image"})["src"]
            flag = 0
            for word in part_name.split(" "):
                if(word not in title.lower().split()):
                    flag = 1
